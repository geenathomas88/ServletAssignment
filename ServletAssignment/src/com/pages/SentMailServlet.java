...
public class SentMailServlet extends HttpServlet{
	...
	void showMail(HttpServletRequest req, HttpServletResponse resp) throws IOException, ServletException{
		...
		try {
			...
			messageHash = getSent(sessionId, con);
			
			req.getRequestDispatcher("header.html").include(req, resp);
			pw.write("<h2 style='color:blue' align= center>Sent Mail</h2>");
	...
	HashMap<Mail, MailContent> getSent(int sessionId, Connection con) throws SQLException{
		HashMap<Mail, MailContent> messages = new HashMap<Mail, MailContent>();
		messages = DBLayer.getSentMail(sessionId, con,1);
		return messages;
	}
