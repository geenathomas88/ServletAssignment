...
public class InboxServlet extends HttpServlet{
	...
	void showMail(HttpServletRequest req, HttpServletResponse resp) throws IOException, ServletException{
		...
		try {
			...
			messageHash = getInbox(sessionId, con);
			
			req.getRequestDispatcher("header.html").include(req, resp);
			pw.write("<h2 style='color:blue' align= center>Inbox</h2>");
			...
			while(messageIterator.hasNext()){
				...
				pw.write("<tr><td>"+mail.get_UserName(mail.getFrom_userid(), con)+"</td>");
	...
	HashMap<Mail, MailContent> getInbox(int sessionId, Connection con) throws SQLException{
		HashMap<Mail, MailContent> messages = new HashMap<Mail, MailContent>();
		messages = DBLayer.getInboxMail(sessionId, con);
		return messages;
	}
}
